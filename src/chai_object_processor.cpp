...
std::stringstream ChaiObjectProcessor::generateRegistrations(const std::string& _namespace, const std::string& location, const bool verbose_output) {
  std::stringstream output;

  std::string uppercase_namespace = _namespace;

  for(auto&& c : uppercase_namespace)
    c = std::toupper(c);

  if(verbose_output)
    std::cout<<"Writing prefix..."<<std::endl;

  output << "#ifndef " << uppercase_namespace << "_REGISTRATIONS_H" << std::endl;
  output << "#define " << uppercase_namespace << "_REGISTRATIONS_H" << std::endl;
  ...
  for(auto filename : filenames) {
    auto f = filename.erase(0, location.size());
    output << "#include \"" << f << "\"" << std::endl;
  }
  if(_namespace != "")
